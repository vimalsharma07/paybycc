...
@section('title', 'Edit gateway — '.config('app.name'))

@section('content')
    @php
        $credDisplay = old('credentials_json');
        if ($credDisplay === null) {
            $credDisplay = json_encode($gateway->credentials ?? [], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
        }
    @endphp

    <div class="mb-8">
        <a href="{{ route('admin.gateways.index') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">← Back to gateways</a>
        <h1 class="mt-4 text-2xl font-semibold text-slate-900">Edit gateway</h1>
        <p class="mt-1 text-sm text-slate-600">{{ $gateway->name }} · <span class="font-mono text-xs">{{ $gateway->code }}</span></p>
    </div>

    <div class="max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <form method="POST" action="{{ route('admin.gateways.update', $gateway) }}" class="space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name', $gateway->name) }}" required maxlength="120"
                ...
                <div>
                    ...
                    <input id="code" name="code" type="text" value="{{ old('code', $gateway->code) }}" required maxlength="64"
                ...
                <div>
                    ...
                    <input id="filename" name="filename" type="text" value="{{ old('filename', $gateway->filename) }}" required maxlength="120"
                ...
                <div>
                    ...
                    <select id="status" name="status" class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                        <option value="active" @selected(old('status', $gateway->status) === 'active')>Active</option>
                        <option value="inactive" @selected(old('status', $gateway->status) === 'inactive')>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 sm:col-span-2">
                    ...
                    <input id="is_primary" name="is_primary" type="checkbox" value="1" class="mt-1 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" @checked(old('is_primary', $gateway->is_primary))>
                    <div>
                        <label for="is_primary" class="text-sm font-medium text-slate-800">Primary gateway</label>
                        <p class="mt-1 text-xs text-slate-600">Unset others when checked.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid gap-5 sm:grid-cols-3">
                <div>
                    ...
                    <input id="min_txn" name="min_txn" type="text" inputmode="decimal" value="{{ old('min_txn', $gateway->min_txn) }}" required
                ...
                <div>
                    ...
                    <input id="max_txn" name="max_txn" type="text" inputmode="decimal" value="{{ old('max_txn', $gateway->max_txn) }}" required
                ...
                <div>
                    ...
                    <input id="daily_limit" name="daily_limit" type="text" inputmode="decimal" value="{{ old('daily_limit', $gateway->daily_limit) }}" required
                    ...
                    <p class="mt-1 text-xs text-slate-500">0 = unlimited.</p>
                    @error('daily_limit')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea id="credentials_json" name="credentials_json" rows="8"
                    class="block w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-xs shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 @error('credentials_json') border-red-500 @enderror">{{ $credDisplay }}</textarea>
                @error('credentials_json')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-2 text-xs text-slate-500">Stored encrypted in the database. Keys depend on your driver (e.g. Cashfree: <span class="font-mono">client_id</span>, <span class="font-mono">client_secret</span>).</p>
            </div>
