{{-- Marketing site: app-style bottom bar on small screens only. --}}
@php
...
@if (! $admin)
    <nav class="mobile-dock fixed bottom-0 left-0 right-0 z-50 border-t border-white/10 bg-slate-950/95 backdrop-blur-md lg:hidden mobile-dock-safe" aria-label="Quick navigation">
        @guest
            <div class="relative mx-auto grid h-[4.25rem] max-w-lg grid-cols-4 items-end px-1 pb-1 pt-0.5">
                ...
                <div class="relative flex justify-center">
                    <a href="{{ route('register') }}" class="mobile-dock-fab absolute -top-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 via-indigo-600 to-violet-600 text-xs font-extrabold text-white shadow-lg shadow-indigo-500/30 ring-4 ring-slate-950 transition hover:brightness-110 active:scale-95" aria-label="Pay now">
                        Pay
                    </a>
                </div>
                ...
                <details class="relative flex flex-col items-center justify-end pb-0.5">
                    <summary class="mobile-dock-item flex cursor-pointer list-none flex-col items-center gap-0.5 pb-1.5 pt-1 text-slate-400 marker:content-none hover:text-white [&::-webkit-details-marker]:hidden">
                    ...
                    <div class="absolute right-0 top-full z-[60] mt-2 w-56 overflow-hidden rounded-xl border border-white/10 bg-slate-900 py-1 shadow-xl ring-1 ring-white/5">
                        <a href="{{ route('home') }}#bills" class="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5">Bill types</a>
                        <a href="{{ route('about') }}" class="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5">About</a>
                        <a href="{{ route('contact') }}" class="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5">Contact</a>
                        <a href="{{ route('privacy') }}" class="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5">Privacy</a>
                        <a href="{{ route('terms') }}" class="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5">Terms</a>
                    </div>
                </details>
            </div>
        @else
            @if ($kyc)
                <div class="relative mx-auto grid h-[4.25rem] max-w-lg grid-cols-5 items-end px-0.5 pb-1 pt-0.5">
                    ...
                    <div class="relative flex justify-center">
                        <a href="{{ route('payments.create') }}" class="mobile-dock-fab absolute -top-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 via-indigo-600 to-violet-600 text-xs font-extrabold text-white shadow-lg shadow-indigo-500/30 ring-4 ring-slate-950 transition hover:brightness-110 active:scale-95 {{ request()->routeIs('payments.*') ? 'ring-cyan-400/50' : '' }}" aria-label="Pay now">
                            Pay
                        </a>
                    </div>
                    ...
                    <details class="relative flex flex-col items-center justify-end pb-0.5">
                        ...
                        <div class="absolute right-0 top-full z-[60] mt-2 w-56 overflow-hidden rounded-xl border border-white/10 bg-slate-900 py-1 shadow-xl ring-1 ring-white/5">
                            <a href="{{ route('dashboard') }}" class="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5">Dashboard</a>
                            <a href="{{ route('about') }}" class="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5">About</a>
                            <a href="{{ route('contact') }}" class="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5">Contact</a>
                            <form method="POST" action="{{ route('logout') }}" class="border-t border-white/10">
                                @csrf
                                <button type="submit" class="w-full px-4 py-2.5 text-left text-sm font-medium text-red-400 hover:bg-red-500/10">Log out</button>
                            </form>
                        </div>
                    </details>
                </div>
            @else
                <div class="relative mx-auto flex h-[4.25rem] max-w-lg items-end justify-between px-6 pb-1 pt-0.5">
                    <a href="{{ route('home') }}" class="mobile-dock-item flex flex-col items-center gap-0.5 pb-1.5 text-slate-400 hover:text-white">
                        <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><path stroke-linecap="round" stroke-linejoin="round" d="M9 22V12h6v10"/></svg>
                        <span class="text-[10px] font-semibold leading-none">Home</span>
                    </a>
                    <a href="{{ route('kyc.index') }}" class="mobile-dock-fab absolute left-1/2 top-1/2 flex h-14 w-14 -translate-x-1/2 -translate-y-[70%] items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 via-indigo-600 to-violet-600 text-xs font-extrabold text-white shadow-lg ring-4 ring-slate-950 transition hover:brightness-110 active:scale-95" aria-label="Complete KYC">
                        KYC
                    </a>
                    <details class="relative flex flex-col items-center pb-0.5">
                        <summary class="mobile-dock-item flex cursor-pointer list-none flex-col items-center gap-0.5 pb-1.5 marker:content-none hover:text-white [&::-webkit-details-marker]:hidden">
                            <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75" aria-hidden="true"><circle cx="5" cy="12" r="1.5" fill="currentColor"/><circle cx="12" cy="12" r="1.5" fill="currentColor"/><circle cx="19" cy="12" r="1.5" fill="currentColor"/></svg>
                            <span class="text-[10px] font-semibold leading-none">More</span>
                        </summary>
                        <div class="absolute right-0 top-full z-[60] mt-2 w-56 overflow-hidden rounded-xl border border-white/10 bg-slate-900 py-1 shadow-xl ring-1 ring-white/5">
