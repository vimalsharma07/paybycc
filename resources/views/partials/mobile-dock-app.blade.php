...
@php
    ...
    $tilesBase = [
        ...
        ['href' => route('wallet.index'), 'label' => 'Wallet', 'icon' => 'wallet'],
    ...
    <nav class="mobile-dock fixed bottom-0 left-0 right-0 z-50 border-t border-slate-200/90 bg-white/95 backdrop-blur-md lg:hidden mobile-dock-safe" aria-label="App navigation">
        @if ($canPayout)
            <div class="relative mx-auto grid min-h-[4.75rem] max-w-lg grid-cols-5 items-end px-0.5 pb-1.5 pt-3">
                ...
                <a href="{{ $u->isSeller() ? route('account.settlements') : route('wallet.index') }}" class="mobile-dock-item flex flex-col items-center justify-end gap-0.5 pb-1.5 pt-1 {{ request()->routeIs('account.settlements', 'wallet.*') ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                    @if ($u->isSeller())
