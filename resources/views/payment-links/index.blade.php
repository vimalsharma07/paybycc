...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <p class="text-sm text-slate-600">Share a link with fixed or open amount. Set one-time, 10-use, or unlimited — with optional expiry.</p>
    ...
    @if ($paymentLinks->isEmpty())
        <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
            ...
            <p class="mt-2 text-sm text-slate-600">Create a fixed or open-amount link and share the URL on WhatsApp, email, or invoice.</p>
                ...
                @foreach ($paymentLinks as $link)
                    @php
                        $statusTone = match ($link->status) {
                            ...
                            \App\Models\PaymentLink::STATUS_EXHAUSTED => 'bg-amber-100 text-amber-900',
                    ...
                    <li class="flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                        <div class="min-w-0">
                            <div class="flex flex-wrap items-center gap-2">
                                <p class="font-mono text-xl font-bold text-slate-900">{{ $link->amountLabel() }}</p>
